...
def webhook_route():
    ...
    try:
        ...
        # ========================================================
        # O SEGREDO DESVENDADO: O Formato Exato do Prompt Lab
        # ========================================================
        payload = {
            "parameters": {
                "prompt_variables": {
        ...
        if response.status_code != 200:
            return jsonify({"response": f"O modelo de IA não respondeu corretamente: {response.text}"}), 200

        result = response.json()
        
        # 3. Extração à prova de falhas
        try:
            # Modelos Prompt Lab costumam retornar em 'results' ou 'predictions'
            if 'results' in result:
                assistant_reply = result['results'][0]['generated_text']
            elif 'predictions' in result:
                assistant_reply = result['predictions'][0]['values'][0][0]
            else:
                assistant_reply = json.dumps(result)
        except Exception:
            assistant_reply = "Não consegui formatar a resposta. JSON Retornado: " + json.dumps(result)
